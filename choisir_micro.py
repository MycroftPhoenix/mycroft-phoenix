#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Script : choisir_micro.py

Permet de choisir un peripherique d'entree (microphone) parmi ceux detects
par sounddevice, et de mettre a jour la configuration de Mycroft-Phoenix.

Usage:
    python choisir_micro.py

Le script :
1. Affiche tous les peripheriques d'entree (microphones) avec leurs indices.
...
def liste_sorties():
    """Renvoie la liste des peripheriques d'entree (microphones)."""
...
def applique_config(device_index, device_name):
    ...
    # On ne modifie que la section input, on conserve les autres champs
    config["input"]["device_index"] = device_index
    config["input"]["name"] = device_name
...
def main():
    ...
    if not dispositifs:
        print("Aucun peripherique d'entree detecte.")
        sys.exit(1)

    print("Peripheriques d'entree audio detects :")
