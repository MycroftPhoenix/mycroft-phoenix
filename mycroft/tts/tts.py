...
from mycroft.util.file_utils import get_temp_path, resolve_resource_file
...
class TTS(metaclass=ABCMeta):
    ...
    def init(self, bus):
        ...
        # Pas d'enclosure physique (mouth/eyes) dans Phoenix — self.enclosure
        # reste None, deja gere par les checks `if self.enclosure:` existants
...
class TTSFactory:
    ...
    from mycroft.tts.piper_tts import PiperTTS
    from mycroft.tts.windows_tts import WindowsTTS
    ...
    def create():
        ...
        """
        from mycroft.tts.tts_selector import patch_tts_config
        config = Configuration.get()
        lang = config.get("lang", "en-us")
        tts_config = patch_tts_config()
        tts_module = tts_config.get('module', 'mimic')
        tts_engine_config = config.get('tts', {}).get(tts_module, {})
        tts_lang = tts_engine_config.get('lang', lang)
        try:
            ...
            tts = clazz(tts_lang, tts_engine_config)
            tts.validator.validate()
        except Exception:
            if tts_module != 'dummy':
                LOG.exception('The selected TTS backend couldn\'t be loaded. '
                              'Falling back to Dummy')
                clazz = TTSFactory.CLASSES.get('dummy')
                tts_engine_config = config.get('tts', {}).get('dummy', {})
                tts = clazz(tts_lang, tts_engine_config)
            else:
