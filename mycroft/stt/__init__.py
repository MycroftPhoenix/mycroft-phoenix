...
class STTFactory:
    ...
    @staticmethod
    def _load_vosk_classes():
        try:
            from mycroft.stt.vosk_stt import VoskSTT, VoskStreamingSTT
            STTFactory.CLASSES["vosk"] = VoskSTT
            STTFactory.CLASSES["vosk_streaming"] = VoskStreamingSTT
        except Exception as e:
            LOG.warning('Could not load Vosk STT: %s', e)

    @staticmethod
    def create():
        from mycroft.stt.stt_selector import patch_stt_config
        STTFactory._load_vosk_classes()
        try:
            config = Configuration.get().get("stt", {})
            stt_config = patch_stt_config()
            module = stt_config.get("module", "mycroft")
